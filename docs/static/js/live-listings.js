    ...
    return `<div style="background:#fff;padding:1.1rem 1.25rem;border-left:4px solid ${t.is_premium ? 'var(--orange,#c05600)' : 'var(--blue,#005ea2)'};border-bottom:1px solid var(--border-lt,#dcdcdc);display:flex;flex-direction:column;gap:.6rem">
      <div style="display:flex;align-items:flex-start;gap:.75rem">
        ...
        <div style="flex:1;min-width:0">
          ...
          ${t.response_time_label ? `<div style="font-size:.72rem;color:#065f46;background:#ecfdf5;border:1px solid #a7f3d0;padding:.1rem .4rem;border-radius:3px;margin-top:.2rem;display:inline-block">⚡ ${esc(t.response_time_label)}</div>` : ''}
          ${(t.nocowboys_rating || t.builderscrack_rating || t.google_rating) ? `<div style="font-size:.72rem;color:#4a5568;margin-top:.2rem;display:flex;gap:.4rem;flex-wrap:wrap">${t.nocowboys_rating ? `<span style="background:#f0fdf4;border:1px solid #bbf7d0;padding:.1rem .4rem;border-radius:3px" title="NoCowboys rating">NC ⭐ ${parseFloat(t.nocowboys_rating).toFixed(1)}${t.nocowboys_reviews ? ` (${esc(String(t.nocowboys_reviews))})` : ''}</span>` : ''}${t.builderscrack_rating ? `<span style="background:#fff7ed;border:1px solid #fed7aa;padding:.1rem .4rem;border-radius:3px" title="Builderscrack rating">BC ⭐ ${parseFloat(t.builderscrack_rating).toFixed(1)}${t.builderscrack_reviews ? ` (${esc(String(t.builderscrack_reviews))})` : ''}</span>` : ''}${t.google_rating ? `<span style="background:#eff6ff;border:1px solid #bfdbfe;padding:.1rem .4rem;border-radius:3px" title="Google rating">G ⭐ ${parseFloat(t.google_rating).toFixed(1)}${t.google_reviews ? ` (${esc(String(t.google_reviews))})` : ''}</span>` : ''}</div>` : ''}
        </div>
      </div>
