"""
TradieTools Wave 5 outreach — untouched high-demand trades.
Targets: gasfitters, kitchen/bathroom renovators, arborists, pest-controllers,
         bricklayers, drainage-specialists, insulation-installers, excavators.
Run: cd outreach && python3 run_outreach_wave5.py
"""
...
OUTPUT   = Path(__file__).parent / "contacts_wave5.csv"
SENT_LOG = Path(__file__).parent / "sent_wave5.log"
LOG_FILE = Path(__file__).parent / "run_outreach_wave5.log"
...
TRADE_LABEL = {
    ...
    "kitchen-renovators":    "kitchen renovator",
    "bathroom-renovators":   "bathroom renovator",
    "arborists":             "arborist",
    "pest-controllers":      "pest controller",
    "bricklayers":           "bricklayer",
    "drainage-specialists":  "drainage specialist",
    "insulation-installers": "insulation installer",
...
<a href="https://tradietools.nz/signup/?ref=claim&id={listing_id}&utm_source=email&utm_medium=email&utm_campaign=wave5" class="btn">Claim your free listing →</a>
...
https://tradietools.nz/signup/?ref=claim&id={listing_id}&utm_source=email&utm_medium=email&utm_campaign=wave5
...
def main():
    LOG_FILE.write_text("=== TradieTools Wave 5 outreach ===\n")
    sent = set(SENT_LOG.read_text().splitlines()) if SENT_LOG.exists() else set()
    # Avoid re-contacting anyone from previous waves
    for prev_log in ["sent.log", "sent_wave2.log", "sent_wave3.log", "sent_wave4.log"]:
        p = Path(__file__).parent / prev_log
        if p.exists():
            sent.update(p.read_text().splitlines())

    listings = get_listings()
    log(f"Targeting {len(listings)} listings (new trades: gasfitters, kitchen/bathroom renovators, arborists, pest-controllers, bricklayers, drainage, insulation, excavators)\n")
    ...
    with sync_playwright() as pw:
        ...
        for i, lst in enumerate(listings):
            ...
            email   = find_email(page, website) if website else None

            status = "no_website"
            if website and not email:
                status = "no_email"
            elif email:
                if email in sent or unsubscribe.is_unsubscribed(email):
                    status = "skip"
                    log(f"  -> skip ({email})")
                elif not daily_limit.under_limit():
                    log(f"  ⏸ daily limit reached — stopping")
                    results.append({
                        "listing_id": lid, "name": name, "trade": trade,
                        "region": region, "website": website or "", "email": email,
                        "status": "limit", "reviews": lst.get("reviews", ""),
                        "rating": lst.get("rating", ""),
                    })
                    break
                else:
                    try:
                        send(email, name, trade, region, lid)
                        status = "sent"
                        sent.add(email)
    ...
    try:
        ...
        summary["Subject"] = f"TradieTools Wave 5 complete — {emailed} emails sent"
...
<h2 style="color:#ea6325">Wave 5 Outreach Complete</h2>
<p><strong>{emailed} cold emails sent</strong> — gasfitters, kitchen/bathroom renovators, arborists, pest-controllers, bricklayers, drainage, insulation, excavators.</p>
