"""
TradieTools Wave 2 outreach — roofers, painters, concreters, tilers.
Run: .venv/bin/python3 outreach/run_outreach_wave2.py
"""
...
PRIORITY_TRADES = ["roofers", "painters", "concreters", "tilers"]
TRADE_LABEL = {
    "roofers":    "roofer",
    "painters":   "painter",
...
<p>There's also a Verified plan ($29/mo) that ranks you higher and sends lead alerts,
...
def main():
    LOG_FILE.write_text("=== TradieTools Wave 2 outreach ===\n")
    ...
    try:
        msg = MIMEMultipart("alternative")
        msg["Subject"] = f"TradieTools Wave 2 complete — {emailed} emails sent"
...
<h2 style="color:#e85d26">Wave 2 Outreach Complete</h2>
<p><strong>{emailed} cold emails sent</strong> — roofers, painters, concreters, tilers.</p>
<p>Processed {len(results)} &bull; Websites {sum(1 for r in results if r.get('website'))} &bull; Emails {sum(1 for r in results if r.get('email'))}</p>
<h3>Sent to:</h3>
