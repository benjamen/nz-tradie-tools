"""
Wave 4 Day 4 follow-up — nudge, make sure it didn't get buried.
Send ~Jul 2 (4 days after Jun 28 wave 4 sends).
Run: venv/bin/python3 outreach/followup_w4_day4.py [--dry-run]
"""
...
FOLLOWUP_LOG = Path(__file__).parent / "followup_w4_day4.log"

DRY_RUN = "--dry-run" in __import__("sys").argv

CAMPAIGN = "followup_w4_day4"
...
def make_email(name, email, trade, region, reviews, rating, listing_id="", slug=""):
...
{"View your live profile: https://tradietools.nz/businesses/" + slug + "/" if slug else ""}
...
<body style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;color:#1e293b;line-height:1.6">
  ...
  <div style="border:1px solid #e2e8f0;border-top:none;padding:1.5rem 1.25rem;border-radius:0 0 6px 6px">
    ...
    {"<p>Here's your live profile on TradieTools:</p><p style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:.75rem 1rem;font-size:.88rem'>🔗 <a href='https://tradietools.nz/businesses/" + slug + "/' style='color:#0055a5'>" + name + " — view your profile</a></p>" if slug else ""}
...
def main():
    ...
    print(f"{'DRY RUN — ' if DRY_RUN else ''}Sending Wave 4 Day 4 follow-ups to {len(rows)} contacts")
    ...
    for row in rows:
        ...
        slug       = row.get("listing_slug", "").strip()
        subject, text, html = make_email(name, email, trade, region, reviews, rating, listing_id, slug)
